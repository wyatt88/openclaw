...
export async function initFederation(
  ...
  // ── Step 10: Register /federation/pair/code-accept route ──
  // This unauthenticated endpoint handles OC- code pairing requests
  // from remote peers. The pairing code challenge/response IS the auth.
  ...
  pairingManager.on("session:completed", (result) => {
    log.info(
      `federation code-pairing completed: peer=${result.peerName} (${formatPeerId(result.peerId)})`,
    );
  });

  if (opts.httpServers.length > 0) {
    registerCodeAcceptRoute(opts.httpServers[0], pairingManager);
    log.info("federation /federation/pair/code-accept route registered");
  }
...
/**
 * Register the `/federation/pair/code-accept` HTTP route on a Gateway server.
 *
 * This is an **unauthenticated** endpoint — the pairing code challenge/response
 * is the authorization mechanism. Remote peers POST here after decoding an
 * OC- pairing code generated by this instance.
 */
function registerCodeAcceptRoute(server: http.Server, pairingManager: PairingManager): void {
  server.prependListener("request", async (req: http.IncomingMessage, res: http.ServerResponse) => {
    const url = req.url ?? "";
    const method = req.method ?? "GET";

    // Only handle POST /federation/pair/code-accept
    if (method !== "POST" || url !== "/federation/pair/code-accept") {
      return;
    }
    ...
    try {
      ...
      sendJsonResponse(res, 400, { ok: false, error: "Invalid JSON body" });
      return;
    }

    // Validate required fields
    const payload = body as {
      identity?: { peerId?: string; publicKeyPem?: string; name?: string };
    ...
    if (
      ...
      sendJsonResponse(res, 400, {
        ok: false,
        error: "Missing required fields: identity, challengeResponse, challenge, timestamp",
      });
      return;
    }

    try {
      const result = pairingManager.handleCodeAcceptRequest({
        identity: {
          peerId: payload.identity.peerId,
          publicKeyPem: payload.identity.publicKeyPem,
          name: payload.identity.name,
        },
        endpoint: (payload.endpoint ?? {}) as import("./types.js").PeerEndpoint,
      ...
      if (!result.ok) {
        ...
        log.warn(`federation code-accept rejected: ${errResult.error}`);
        sendJsonResponse(res, 403, { ok: false, error: errResult.error });
        return;
      }

      log.info(
        `federation code-accept succeeded: peer=${payload.identity.name} (${payload.identity.peerId.slice(0, 8)}...)`,
      );
      ...
      log.error(`federation code-accept error: ${String(err)}`);
  ...
  server.prependListener("request", (req: http.IncomingMessage, res: http.ServerResponse) => {
    if (req.method === "OPTIONS" && req.url === "/federation/pair/code-accept") {
      res.setHeader("Access-Control-Allow-Origin", "*");
      res.setHeader("Access-Control-Allow-Methods", "POST, OPTIONS");
      res.setHeader("Access-Control-Allow-Headers", "Content-Type");
      res.writeHead(204);
